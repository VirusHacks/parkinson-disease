"""
Evaluation and reporting utilities for the upgraded Parkinson's Motor environment.

This script:
  - runs baseline policies on public and held-out scenario sets
  - summarizes mean/std scores and success rates
  - writes machine-readable and markdown reports into ../outputs/
"""
...
def safety_aware_policy(obs) -> ParkinsonsMotorAction:
    """Simple adaptive baseline that changes behavior by task and side-effect load."""
    beta = obs.beta_arv
...
def reference_policy(obs) -> ParkinsonsMotorAction:
    """
    Reference-style controller that uses the published controller amplitude as a
    hint, then adds mild adaptation from the current observation.
    """
...
def build_markdown(public: Dict[str, Dict[str, object]], heldout: Dict[str, Dict[str, object]]) -> str:
    lines: List[str] = []
    lines.append("# Benchmark Evaluation Report")
    lines.append("")
    lines.append("This report is auto-generated by `parkinsons_Motor/eval_suite.py`.")
    lines.append("")
    ...
    lines.append("")
    lines.append("- `no_dbs` should fail all tasks.")
    ...
    lines.append("- Held-out fragile/refractory cases should remain clearly harder than public runs.")
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    MD_PATH.write_text(build_markdown(public, heldout), encoding="utf-8")
