...
for task_id in ["beta_suppression", "tremor_correction", "full_episode"]:
    ...
    print(f"Task: {task_id} | n_steps={n} | force={obs.force_preserved:.3f} | beta={obs.beta_arv:.3f}")

    total_reward = 0.0
    for _ in range(n):
        action = ParkinsonsMotorAction(motor_command=0.3, dbs_amplitude=1.0, dbs_pulse_width=0.13)
        obs = env.step(action)
        total_reward += obs.reward or 0.0
        if obs.done:
            break

    print(f"  -> total_reward={total_reward:.2f} | grader_score={obs.grader_score:.4f} | success={obs.episode_success}")
